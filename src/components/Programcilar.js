...
const [ondekininid,setondekininid] = useState();
  ...
  const bulunanIsimler = programcilarList.find ((dev) => dev.id === ondekininid)
      ...
      <div className='programmers'>
        {
          ...
          programcilarList.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.isim} {" "} <button onClick={() => {/* burada dev.id 'yi öne çıkan id'ye atayın */setondekininid(dev.id) }}>Kutla</button>
            </div>
          )
        }
      </div>
